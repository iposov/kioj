...
\usepackage[eqsecnum,authorsinfo]{kioj3}
% Здесь можно подключить свои пакеты. Некоторые пакеты работают только если их подключить перед предыдущей строкой \usepackage[eqsecnum]{kioj3}
% \usepackage[•]{•}
\usepackage{array}
...
\authorIinfo{информация об авторе}
\authorIemailEnvelope{aaa@org.com}
...
\authorIIinfo{информация о втором авторе}
...
\begin{document}
    ...
    \begin{translatedpart}
        ...
        \authorIpos{position position position position position,
        institution institution institution institution institution}
        \authorIinfo{1st author info}

        \authorII[Borisov B.~B.\affil{2}]{Borisov Boris Borisovich}
        \authorIIpos{just a cool guy with some position at some institution}
        \authorIIinfo{2nd author info}
